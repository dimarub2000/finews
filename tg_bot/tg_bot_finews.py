...
def get_text_messages(message):
    ...
    if message.text == "/commands":
        ...
        state = 1
        bot.send_message(message.from_user.id, "Впиши тикер компании которая тебя интересует (если интересуют новости по всем компаниям сразу напиши -).")
...
def get_limit(message):
    ...
    for item in data:
        ...
        " время публикации: {}, ссылка на источник: {}".format(item['tags'], item['time'], item['link']),
    ...
    if len(data) == 0:
        bot.send_message(message.from_user.id, "Ничего не найдено по такому запросу, либо не валидный лимит"
                " (попробуйте любое положительное число), либо нет такого тикера на данный момен ("
                " чтобы посмотреть доступные на данный момент тикеры, напиши мне команду: tickers)")
